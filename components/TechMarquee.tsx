...
export const TechMarquee: React.FC = () => {
  return (
    <Card className="w-full h-full overflow-hidden" title="Tech Stack" noPadding>
      <div className="flex items-center h-full bg-white dark:bg-zinc-900 relative overflow-hidden transition-colors duration-300">
        <motion.div 
        ...
        >
          ...
          {[...TECH_STACK, ...TECH_STACK, ...TECH_STACK].map((tech, idx) => (
            <div key={idx} className="flex flex-col items-center justify-center gap-2 cursor-default w-24">
              <tech.icon className="text-4xl text-gray-600 dark:text-gray-300" />
              <span className="font-body text-sm text-gray-800 dark:text-gray-200 font-semibold truncate w-full text-center">
                {tech.name}
              </span>
            </div>
          ))}
